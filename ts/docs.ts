  ...
  await Promise.all(
    schemas.map(async jsonFile => {
      try {
        ...
        const maybeMdFile = path.join('schema', `${title}.md`)
        const mdFileExists = await fs.pathExists(maybeMdFile)
        const mdFile = mdFileExists ? maybeMdFile : schemaMdFile
        ...
        if (!isArticle(article)) {
          log.warn(
            `docs.ts#57: cant decode '${mdFile}' as 'Article'; skipping HTML generation`
          )
          return
        }
          ...
          ? [
              {
                ...
                depth: 6,
                content: [
                  `note: this html file was autogenerated by 'ts/docs.ts::schema2Article()' from '${schemaMdFile}'; create documentation for it at '${maybeMdFile}'`
                ],
                id: 'notice'
              }
            ]
